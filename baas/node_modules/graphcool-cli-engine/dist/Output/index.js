...
exports.CustomColors = {
    ...
    app: function (s) { return exports.CustomColors.graphcool("\u2B22 " + s); },
    graphcool: function (s) {
...
var Output = (function () {
    function Output(config) {
        var _this = this;
        this.mock = true;
        this.printServices = function (targets, projects, onlyLocal) {
            if (onlyLocal === void 0) { onlyLocal = true; }
            var uniqTargetKeys = lodash_1.uniqBy(Object.keys(targets), function (key) { return targets[key].id; });
            if (onlyLocal) {
                return _this.printPadded(uniqTargetKeys.map(function (key) {
                    var _a = targets[key], id = _a.id, cluster = _a.cluster;
                    var project = projects.find(function (p) { return p.id === id; });
                    var output = cluster + "/" + id;
                    var serviceName = project ? project.name : key;
                    return [serviceName + '  ', output + '  '];
                }), 0, 1, ['Service Name', 'Cluster / Service ID']);
            }
            else {
                var filteredProjects = projects.filter(function (p) { return !lodash_1.values(targets).find(function (t) { return p.id === t.id; }); });
                return _this.printPadded(uniqTargetKeys
                    .map(function (key) {
                    var _a = targets[key], id = _a.id, cluster = _a.cluster;
                    var project = projects.find(function (p) { return p.id === id; });
                    var output = cluster + "/" + id;
                    ...
                    .concat(filteredProjects.map(function (p) {
                    return [
                        p.name,
                        "shared-" + p.region.toLowerCase().replace(/_/g, '-') + "/" + p.id,
                    ];
                }))
                    .map(function (l) { return [l[0] + '  ', l[1] + '  ']; }), 0, 1, ['Service Name', 'Cluster / Service ID']);
            }
        };
        this.config = config;
